...
df_grid$sharpe_ratio = NA
df_grid$sortino_ratio = NA
df_grid$max_drawdown = NA
df_grid$win_rate = NA
df_grid$knockout_rate = NA
df_grid$information_ratio = NA
...
stop("TODO: Store more intermediate results, i.e. portfolio time series, knockouts, etc. to detect common patterns of success/failure!")
...
for (i in 1:nrow(m_grid)) {
    ...
    for (j in v_date) {
        ...
        # Observe increased vola in last n days by measuring number of days where close price dropped more than x%
        ...
        # Observe number of knockouts in last n days -> if exceeded -> do not buy - and liquidate all long positions
        ...
        if (risk_n_days_signal_TF == 1 & count_n_days_signal_cutoff >= risk_n_days_signal_threshold) {
            ...
            buy_value_j = min(
                m_portfolio[j, "cash_bop"],
                max(
                    m_portfolio[j, "cash_bop"] * liquidity_inv_perc,
                    liquidity_init * liquidity_inv_perc
                )
            )
    ...
    # Filter matrices to relevant rows
    m_positions = m_positions[m_positions[,"buy_date"] > 0, ]
    ...
    # !!! TODO: Is it fair to compare realized portfolio returns with index valuations????


    # Calculate portfolio returns for performance metrics
    portfolio_cash = m_portfolio[,"cash_eop"]
    portfolio_returns = diff(portfolio_cash) / portfolio_cash[-length(portfolio_cash)]
    portfolio_returns = portfolio_returns[!is.na(portfolio_returns)]
    ...
    if (length(portfolio_returns) > 0) {
        ...
        if (sd(portfolio_returns, na.rm = TRUE) > 0) {
            sharpe_ratio = mean(excess_returns, na.rm = TRUE) / sd(portfolio_returns, na.rm = TRUE) * sqrt(252)
        } else {
            sharpe_ratio = 0
        }
        ...
        if (length(downside_returns) > 0) {
            ...
            if (downside_deviation > 0) {
                sortino_ratio = mean(excess_returns, na.rm = TRUE) / downside_deviation * sqrt(252)
            } else {
                sortino_ratio = 0
            }
        } else {
        ...
        # Calculate benchmark returns (buy and hold index)
        index_returns = diff(m_train[,"close"]) / m_train[-nrow(m_train),"close"]
        index_returns = index_returns[!is.na(index_returns)]
        
        # Match lengths
        min_length = min(length(portfolio_returns), length(index_returns))
        if (min_length > 0) {
            port_ret_matched = portfolio_returns[1:min_length]
            index_ret_matched = index_returns[1:min_length]
            
            active_returns = port_ret_matched - index_ret_matched
            if (sd(active_returns, na.rm = TRUE) > 0) {
                information_ratio = mean(active_returns, na.rm = TRUE) / sd(active_returns, na.rm = TRUE) * sqrt(252)
            } else {
                information_ratio = 0
            }
        } else {
    ...
    # % knocked out
    knockout_perc = sum(m_positions[,"default"] == 1) / nrow(m_positions[m_positions[,"buy_n"] > 0, ]) * 100

    # % profitable trades
    profitable_perc = sum(m_positions[,"profit"] > 0) / nrow(m_positions[m_positions[,"buy_n"] > 0, ]) * 100
    
    # Store performance metrics
    m_grid[i, "sharpe_ratio"] = sharpe_ratio
    m_grid[i, "sortino_ratio"] = sortino_ratio
    m_grid[i, "max_drawdown"] = max_drawdown
    m_grid[i, "win_rate"] = profitable_perc
    m_grid[i, "knockout_rate"] = knockout_perc
    m_grid[i, "information_ratio"] = information_ratio
...
# Display top strategies by final liquidity
df_grid %>% 
    cbind(bench_return = bench_return) %>%
    mutate(d_ret_bench = avg_return - bench_return) %>%
    arrange(desc(liquidity_final)) %>%
    select(buy_leverage, holding_period, liquidity_inv_perc, risk_n_days_signal_TF, 
           risk_n_days_signal_threshold, sharpe_ratio, sortino_ratio, max_drawdown, 
           win_rate, knockout_rate, liquidity_final, avg_return, d_ret_bench) %>%
    head(n = 20)

# Plot avg_return vs parameters
df_grid %>%
    select(buy_leverage, holding_period, liquidity_inv_perc, risk_n_days_signal_TF, 
           risk_n_days_signal_threshold, avg_return) %>%
    pivot_longer(cols = -avg_return, names_to = "parameter", values_to = "value") %>%
    ggplot(aes(x = value, y = avg_return)) +
    geom_point() +
...
# Run linear regression with y = liquidity_final and x = all parameters (including new metrics)
lm_fit = lm(liquidity_final ~ buy_leverage + holding_period + liquidity_inv_perc + 
               risk_n_days_signal_TF + risk_n_days_signal + risk_n_days_signal_cutoff + 
               risk_n_days_signal_threshold, 
               data = df_grid)
summary(lm_fit)
